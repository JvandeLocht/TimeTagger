...
void DatabaseManager::printTableShell() {
  string command = "sqlite3 -cmd '.mode box' -cmd '.headers on' " + filepath_ +
...
bool DatabaseManager::populateDailyHours() {
  ...
  if (sqlite3_prepare_v2(db_, sqlDates, -1, &stmtDates, nullptr) != SQLITE_OK) {
    return false;
  }
  ...
  for (const auto &day : dates) {
    ...
    if (sqlite3_prepare_v2(db_, sqlDailyHours, -1, &stmtDailyHours, nullptr) ==
        SQLITE_OK) {
      sqlite3_bind_text(stmtDailyHours, 1, day.c_str(), -1, SQLITE_STATIC);
      sqlite3_bind_text(stmtDailyHours, 2, workHour.c_str(), -1, SQLITE_STATIC);
