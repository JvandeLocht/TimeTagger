...
void DatabaseManager::printTimestampsTable() {
  ...
  const char *sql = "SELECT id, timezone, timestamp, type, created_at "
                    "FROM timestamps "
  ...
  try {
    ...
    Table table;
    table.add_row({"ID", "Timezone", "Timestamp", "Type", "Created At"});
    ...
    while (query.executeStep()) {
      table.add_row({
          std::to_string(query.getColumn(0).getInt()), // id
          query.getColumn(1).getText(),                // timezone
          query.getColumn(2).getText(),                // timestamp
          query.getColumn(3).getText(),                // type
          query.getColumn(4).getText()                 // created_at
      });
